...
export default function WorkoutForm({ workout, onSave, onClose }: WorkoutFormProps) {
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 bg-[#111111]">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <label className="text-sm font-semibold text-[#ededed]">Exercises</label>
            <button
              type="button"
              onClick={addExercise}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600/20 text-blue-400 rounded-lg hover:bg-blue-600/30 text-sm font-medium transition-colors"
            >
          ...
          {exercises.map((exercise, exIndex) => (
            <div key={exercise.id} className="border border-[#333333] rounded-xl p-4 bg-[#1a1a1a]">
              ...
              <div className="flex gap-3 mb-4">
                <div className="flex-1">
                  <input
                    ...
                    className="w-full px-3 py-2 border border-[#333333] rounded-lg focus:ring-2 focus:ring-blue-500 bg-black text-[#ededed] placeholder-gray-500 font-medium"
                  />
                </div>
                <button
                  type="button"
              ...
              <div className="space-y-3">
                ...
                <div className="hidden sm:grid grid-cols-12 gap-3 px-1 text-xs font-medium text-gray-400 uppercase tracking-wider">
                ...
                {exercise.sets.map((set, idx) => (
                  <div key={set.id} className="grid grid-cols-12 gap-2 sm:gap-3 items-center">
                    ...
                    <div className="col-span-12 sm:col-span-2 flex items-center gap-2 sm:block">
                      ...
                      <div className="flex items-center justify-center w-6 h-6 rounded-full bg-[#333333] text-xs font-bold text-[#ededed]">
                        {idx + 1}
                      </div>
                    </div>
                    ...
                    <div className="col-span-5 sm:col-span-4 relative">
                      <input
                        ...
                        placeholder="0"
                        className="w-full pl-3 pr-8 py-2 border border-[#333333] rounded-lg focus:ring-2 focus:ring-blue-500 bg-black text-[#ededed] text-center sm:text-left"
                        step="0.5"
                      />
                    ...
                    <div className="col-span-5 sm:col-span-4 relative">
                      <input
                        ...
                        className="w-full pl-3 pr-8 py-2 border border-[#333333] rounded-lg focus:ring-2 focus:ring-blue-500 bg-black text-[#ededed] text-center sm:text-left"
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-500 pointer-events-none">reps</span>
                    </div>
              ...
              {/* Actions Footer */}
              <div className="mt-4 pt-3 border-t border-[#333333] flex flex-col sm:flex-row gap-3">
                <button
                  type="button"
                  onClick={() => addSet(exercise.id)}
                  className="text-sm text-blue-400 hover:text-blue-300 font-semibold py-1 px-2 rounded hover:bg-blue-500/10 self-start"
                >
                  + Add Set
                </button>
                ...
                <input
                  ...
                  className="flex-1 text-sm px-3 py-1.5 border-0 border-b border-[#333333] focus:border-blue-500 focus:ring-0 bg-transparent text-gray-400 placeholder-gray-500"
                />
              </div>
            </div>
          ))}
          ...
          {exercises.length === 0 && (
            <div className="text-center py-8 border-2 border-dashed border-[#333333] rounded-xl">
              <p className="text-gray-400 text-sm">No exercises added yet.</p>
              <button onClick={addExercise} className="mt-2 text-blue-400 font-medium text-sm">
      ...
      {/* Sticky Footer */}
      <div className="p-4 border-t border-[#333333] bg-[#1a1a1a] flex justify-end shrink-0 rounded-b-xl">
        <button
          type="button"
          onClick={handleSubmit}
          className="flex justify-center items-center gap-2 px-4 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 font-semibold transition-all active:scale-[0.98]"
        >
          <Save size={18} />
          Save Workout
        </button>
